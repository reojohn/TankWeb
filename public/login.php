...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // ==================================================
        ...
        if ($user_id) {
            ...
            try {
                $stmt = $pdo->prepare(
                    'SELECT ' . $policyField . ', ' . $factorTypeField . ',
            ...
            $secondFactorType = $schoolId ? fortress_second_factor_type_value($schoolId) : 'personal_id';
            ...
            elseif (!(bool)$schoolId['school_id_2fa_required']) {
                ...
                if ($wantsJson) {
                    fortress_login_json([
                        ...
                        'verified' => true,
                        'stage' => 'authenticated',
                        'message' => 'Administrator password verified. QR-based 2FA is disabled for this account.',
                        'next_step' => 'dashboard',
                        'redirect' => $redirect,
                        'verification' => [
                            'network' => 'passed',
            ...
            // ==================================================
            // 2FA REQUIRED BUT NO QR CREDENTIAL IS AVAILABLE
            // Personal-ID accounts may self-enroll after password verification.
            // Administrator-issued QR accounts must be regenerated by an admin.
            // ==================================================
            elseif (
                !(bool)$schoolId['school_id_qr_enabled'] ||
                empty($schoolId['school_id_qr_hash'])
            ) {
                if ($secondFactorType === 'generated_qr') {
                    audit_log('issued_qr_missing uid=' . (int)$user_id);
                    unset(
                        $_SESSION['pending_user_id'],
                        $_SESSION['pending_school_id_verification'],
                        $_SESSION['password_verified_at']
                    ...
                    if ($wantsJson) {
                        fortress_login_json([
                            'success' => false,
                            'verified' => true,
                            'stage' => 'second_factor_profile',
                            'message' => 'Password verified, but this account does not have an active issued QR credential. Contact the main administrator to regenerate it.',
                            'next_step' => 'administrator_support',
                            'verification' => [
                                'network' => 'passed',
                                'input_security' => 'passed',
                                'csrf' => 'passed',
                                'bruteforce' => 'passed',
                                'password' => 'passed',
                                'personal_id_profile' => 'issued_qr_missing',
                            ],
                        ], 409);
                    }

                    $error = 'Password verified, but this account needs a new administrator-issued QR credential.';
                    $error_class = 'ban';
                } else {
                    audit_log('school_id_enrollment_required uid=' . (int)$user_id);
